...
import { buildReminderKeyboard } from "./rsvpInline.js";
...
function renderTeamsTwoColsHtml(teamAPlayers, teamBPlayers) {
  ...
  const MAX_TOTAL = 35; // попробуй 42..46 по вкусу
...
Открыть мини-приложение для отметок:`;
  ...
  const kb = buildReminderKeyboard({ deepLink });
...
app.get('/api/chat/conversations', async (req, res) => {
  try {
    ...
    const rr = await q(
      ...
      LEFT JOIN LATERAL (
        SELECT cr.last_read_id
        FROM chat_reads cr
        WHERE cr.conversation_id = c.id
          AND cr.tg_id = CASE
            WHEN c.kind='dm' AND c.user_a=$1 THEN c.user_b
            WHEN c.kind='dm' AND c.user_b=$1 THEN c.user_a
            ELSE NULL
          END
        LIMIT 1
      ) pr ON TRUE
